    ...
    return {
      ...
      kakao: function (clientId) {
        ...
        if (window.cordova) {
          var cordovaMetadata = cordova.require("cordova/plugin_list").metadata;
          if (cordovaMetadata.hasOwnProperty("cordova-plugin-inappbrowser") === true) {
            var browserRef = window.open('https://kauth.kakao.com/oauth/token?client_id=' + clientId + '&redirect_uri=http://localhost/callback&response_type=code','_blank', 'location=no,clearsessioncache=yes,clearcache=yes');
            browserRef.addEventListener('loadstart', function (event) {
              if ((event.url).indexOf("http://localhost/callback") === 0) {
                requestToken = (event.url).split("code=")[1];
                $http.defaults.headers.post['Content-Type'] = 'application/x-www-form-urlencoded';
                $http({
                  ...
                  data: "client_id=" + clientId + "&redirect_uri=http://localhost/callback" +
